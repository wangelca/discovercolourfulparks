...
const EventDetails = () => {
    ...
    const loadReviews = async () => {
        try {
            // Fetch reviews for the event directly from API
            const reviewsResponse = await axios.get(`${API_URL}/reviews/event/${eventId}`);
            setReviews(reviewsResponse.data);

            // Fetch average rating directly from API
            const ratingResponse = await axios.get(`${API_URL}/ratings/event/${eventId}`);
